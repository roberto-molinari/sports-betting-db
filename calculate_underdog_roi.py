...
for match in matches:
    ...
    if home_ml > away_ml:
        underdog_ml = home_ml
        underdog_team = match['home_team']
        underdog_won = home_score > away_score
    else:
        underdog_ml = away_ml
        underdog_team = match['away_team']
        underdog_won = away_score > home_score

    total_wagered += 100

    if underdog_won:
        payout = calculate_payout(100, underdog_ml)
        profit = payout - 100
        total_profit += profit
        wins += 1
        outcome = "WIN"
    ...
    win_details.append({
        ...
        'underdog': f"{underdog_team} ({underdog_ml})",
...
print("=" * 70)
print("NHL MONEYLINE UNDERDOG BETTING ANALYSIS")
print("=" * 70)
